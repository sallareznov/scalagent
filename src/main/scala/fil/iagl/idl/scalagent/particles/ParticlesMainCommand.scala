...
object ParticlesMainCommand extends Command(name = "particles", description = "simulates a bubble chamber using a multi-agent approach") with CommandWithHandling {

  // TODO print default parameters
  ...
  var visible = opt[Boolean](name = "visible", description = "if activated, the plots of the grid will be visible")
  ...
  override def handleCommand(args: Array[String]) {
    Cli.parse(args).withCommand(ParticlesMainCommand.this) {
      case opt => ParticlesView.main(args)
    }
  }
