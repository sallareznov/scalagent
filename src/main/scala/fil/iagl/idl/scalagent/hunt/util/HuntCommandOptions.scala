...
object HuntCommandOptions extends Command(name="hunt", description="a prey, guided by the user's directions, is chased by a defined number of predators, guided by Dijkstra's algorithm") {
  ...
  var speedRatio = opt[Double](name = "speedRatio", default = 1, description = "the speed ratio between the prey and the predators. Should be a positive number (i.e. speedRatio > 1 (ex: 1.5) in favor of prey) [default = 1]")
